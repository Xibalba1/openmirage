...
function readRemoteEnvFile(options) {
  return runChecked(
    "ssh",
    createSshArgs(options, `cat "${options.deployDir}/.env.staging"`)
  );
}
...
try {
  ...
  const failures = [];
  const expectedEnv = expectedStagingEnvEntries(options);
  ...
  log("verifying remote .env.staging content");
  ...
  for (const [key, expectedValue] of Object.entries(expectedEnv)) {
    if (key === "SENTRY_DSN") {
      if (remoteEnv[key] === undefined) {
        failures.push("SENTRY_DSN is missing from .env.staging");
      }
      continue;
    }
  ...
  if (failures.length > 0) {
    console.error("[openmirage] staging prerequisite verification failed:");
    for (const failure of failures) {
      console.error(`[openmirage] - ${failure}`);
    }
    process.exitCode = 1;
  } else {
    log("staging prerequisite verification passed");
  }
} catch (error) {
  const message = error instanceof Error ? error.message : String(error);
  console.error(`[openmirage] staging prerequisite verification failed: ${message}`);
  process.exitCode = 1;
}
