...
function printUsage() {
  ...
  node ./scripts/setup-production-prereqs.mjs \\
    ...
    --public-base-url <https://openmirage.example.com> \\
    [--port 22] [--ssh-key-path ~/.ssh/id_ed25519] [--force]

Environment fallback:
  VPS_HOST, VPS_USER, VPS_PORT, VPS_DEPLOY_DIR, PRODUCTION_PUBLIC_BASE_URL,
...
try {
  ...
  const envFileContent = `${renderProductionEnvFile(options)}\n`;
  const envFilePath = `${options.deployDir}/.env.production`;
  ...
  const tempDir = mkdtempSync(path.join(os.tmpdir(), "openmirage-production-"));
  const localEnvFile = path.join(tempDir, ".env.production");
