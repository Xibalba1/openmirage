...
export function readStagingPrereqOptions(argv = process.argv.slice(2)) {
  ...
  if (!publicBaseUrl) {
    throw new Error(
      "missing staging public base url; set --public-base-url or STAGING_PUBLIC_BASE_URL"
    );
  }
  ...
  if (parsedPublicBaseUrl.protocol !== "https:") {
    throw new Error("staging public base url must start with https://");
  }
...
export function renderStagingEnvFile(options) {
...
export function expectedStagingEnvEntries(options) {
  return parseEnvFile(renderStagingEnvFile(options));
}
