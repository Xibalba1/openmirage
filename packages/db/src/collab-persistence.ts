...
export async function seedPageDocument(
  pageId: string,
  pageDocument: PageDocumentDto,
  poolOrClient: Queryable | Connectable | QueryableClient
): Promise<number> {
  const document = new Y.Doc();
  const pageMap = document.getMap("page");

  pageMap.set("nodes", pageDocument.nodes);
  pageMap.set("rootNodeIds", pageDocument.rootNodeIds);

  const persistence = new PgCollabPersistence(poolOrClient);
  return persistence.appendUpdate(pageId, Y.encodeStateAsUpdate(document));
}
