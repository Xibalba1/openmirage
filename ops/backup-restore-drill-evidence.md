...
- Backup artifact directory: `/tmp/openmirage-backups/openmirage-backup-2026-04-14T13-05-10Z`
- Storage provider: `minio`
- Commands run:
  - `pnpm lint`
  - `docker compose up --build --wait`
  - `BACKUP_ROOT=/tmp/openmirage-backups pnpm backup:create`
  - `BACKUP_ARTIFACT_DIR=/tmp/openmirage-backups/openmirage-backup-2026-04-14T13-05-10Z pnpm backup:verify`
  - `docker compose down --remove-orphans`
  - `BACKUP_ARTIFACT_DIR=/tmp/openmirage-backups/openmirage-backup-2026-04-14T13-05-10Z pnpm verify:recovery:prereqs restore-drill`
  - `BACKUP_ARTIFACT_DIR=/tmp/openmirage-backups/openmirage-backup-2026-04-14T13-05-10Z OPENMIRAGE_RECOVERY_ALLOW_DESTRUCTIVE=true pnpm backup:restore:drill`
- Verification result:
...
- Notes:
  - Initial implementation failed because the MinIO backup archived the full `/data` tree, including `.minio.sys`. The final implementation now archives bucket contents only and waits for `minio-init` to complete before restoring those contents into the clean target.
